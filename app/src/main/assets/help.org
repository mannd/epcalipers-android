...
* Limitations
- Images taken using the *Camera* button may not be automatically saved to your device, depending on your device and Android version.  If you need to save the image, take it with the camera outside of the app and then use the *Select* button to load it into the app.
- Rotation of the device does not preserve the relationship between the image and the calipers, although calibration is conserved with rotation.
- If the image itself is rotated, you should recalibrate your calipers, though the app doesn’t enforce this.  It is not necessary to recalibrate after zooming the image, however.
